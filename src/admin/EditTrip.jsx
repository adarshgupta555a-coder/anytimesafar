...
const EditTrip = () => {
  ...
  const [TripOg, setTog] = useState(null);
  ...
  const getData = async () => {
    let { data: travel, error } = await supabase
      .from('travel')
      .select("*")
      .eq('id', id)
    ...
    if (!error) {
      console.log(travel);
      setTrip({
        status: travel[0]?.status,
        price: travel[0]?.price,
        seats: travel[0]?.seat,
        payment: travel[0]?.payment_verified
  ...
  const OnhandleSubmit = async (e) => {
    ...
    const UpdateTrip = {};
    if (trip?.status && trip?.status !== TripOg?.status) {
      UpdateTrip.status = trip?.status
    }

    if (trip?.price && trip?.price !== TripOg?.price) {
      UpdateTrip.price = trip?.price
    }

    if (trip?.seats && trip?.seats !== TripOg?.seat) {
      UpdateTrip.seat = trip?.seats
    }

    if (Object.keys(UpdateTrip).length === 0) {
      toastError("not update you do.")
      return;
    }
    ...
    if (!error) {
      console.log(data)
      toastSuccess("Travel Updated successfully")
    } 
      ...
      <div className="main-grid">
        ...
        <div>
          <div className="form-section">
            ...
            <form className="edit-form" onSubmit={OnhandleSubmit}>
              {/* Booking Status */}
              <div className="form-group" >
                ...
                <div className="input-wrapper">
                  ...
                  <select name="status" required value={trip?.status} onChange={OnHandlechange}>
              ...
              <div className="form-group">
                ...
                <div className="input-wrapper">
                  <span className="input-icon">💺</span>
                  <input type="text" name="seats" onChange={OnHandlechange}
                    value={trip?.seats} required />
                </div>
              ...
              <div className="form-group">
                ...
                <div className="toggle-group">
                  ...
                  <label className="toggle-switch">
                    <input type="checkbox" name='payment' checked={trip?.payment || false} />
